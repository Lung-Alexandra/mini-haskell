...
fromLeft def_val  = either id (\x -> def_val)
...
fromRight def_val  = either (const def_val) id
...
eitherLeftMap :: EitherClass e => (a -> a1) -> e a b -> e a1 b
eitherLeftMap f ea = either (\x -> left (f x)) (\x -> right x) ea

-- >>> eitherLeftMap not (left true) :: Either.Either CBool CBool
-- Left CFalse

-- | The 'right' 'Functor' instance for 'EitherClass'
eitherRightMap :: EitherClass e => (b -> b1) -> e a b -> e a b1
eitherRightMap f eb = either (\x -> left x) (\x -> right (f x))  eb
